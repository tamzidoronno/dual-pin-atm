<template>
  <div class="submit-form form text-center">
    <div v-if="!submitted">
      ...
      <div class="form-group">
        ...
        <input type="text" class="form-control" required v-model="user.securityBalance" />
      </div>
...
export default {
  ...
  methods: {
    ...
    validateUser() {
      ...
      if(this.user.balance < 500 || this.user.securityBalance< 500)
        return 'Miminum deposit is 500'
      if(+this.user.balance < +this.user.securityBalance)
        return 'Security deposit cannot be greater than initial deposit'
      return '';
    }
  }
}
